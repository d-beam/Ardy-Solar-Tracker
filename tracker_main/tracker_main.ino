...
// tracking will deactivate when the sun is calcualted to be outside these ranges
...
#define MOTOR_SPEED_SLOW_PER_SEC_X      ((MOTOR_SPEED_FAST_PER_SEC_Y * POLOLU_SPEED_SLOW_Y) / POLOLU_SPEED_FAST_Y)    // speed ratiomteric to Speed command?
...
#define CLOSED_LOOP_FAST_MOVE_DEADBAND_X  500 // in raw diff ADC units.
// Big setting to allow mechanism to decellerate and not overshoot
#define CLOSED_LOOP_CONTROL_DEADBAND_X    50    // in raw diff ADC units
#define CLOSED_LOOP_ABORT_LIMIT_X     25    // °

#define OPEN_LOOP_FAST_MOVE_DEADBAND_Y    2   // °
#define OPEN_LOOP_CONTROL_DEADBAND_Y    0.3   // °
#define CLOSED_LOOP_FAST_MOVE_DEADBAND_Y  500   // in raw diff ADC units.  
#define CLOSED_LOOP_CONTROL_DEADBAND_Y    50    // in raw diff ADC units
#define CLOSED_LOOP_ABORT_LIMIT_Y     25


// movement delays: -------------------
// after stopping drive motors, how long to wait for mechanism to stop
#define MOVE_STOP_FROM_FAST_DELAY (1000)    // ms
#define MOVE_STOP_FROM_SLOW_DELAY (1000)    // ms

// Closed loop tracking: -------------------
#define TRACKING_STABLE_COUNT       5   // count of loops before stable declared
...
//compensating for wiring error:
#define DI_LIMIT_SWITCH_Y_MAX 25
#define DI_LIMIT_SWITCH_Y_MIN   22
...
// Pocolu shield, timer1 can be used for PWM:  M1 9, M2 10
...
void arduino_init() {
  ...
  pinMode(DI_MODE_AUTO, INPUT_PULLUP);
...
/* development model
  #define POLOLU_SPEED_FAST_X   (400) // Pocolu max 400
  #define POLOLU_SPEED_SLOW_X   (25)  // Pocolu max 400
  #define POLOLU_SPEED_FAST_Y   (400) // Pocolu max 400
  #define POLOLU_SPEED_SLOW_Y   (25)  // Pocolu max 400
*/
/* Tamera FixFocus */
// Pocolu maxinum 400
...
/******************************************************************************
    Checks for motor faults and handles as required.  Based on demo from Pocolu.
    by:     Gregory Fung <gwfung@gmail.com>
...
void driveX(bool direction, int speed) {
  ...
  if (speed >= 2) {
    pocoluSpeed = POLOLU_SPEED_FAST_X;
  } else if (speed == 1) {
    pocoluSpeed = POLOLU_SPEED_SLOW_X;
  } else {
    pocoluSpeed = 0;
  }
  ...
  motorSettingX = speed;
  if (!direction) {
    pocoluSpeed = -1 * pocoluSpeed;
    motorSettingX = -1 * speed;
  }

  // drive motor
  md.setM1Speed(pocoluSpeed);
...
void driveY(bool direction, int speed) {
  ...
  // TODO consider speed ramp up and ramp down?
  if (speed >= 2) {
    pocoluSpeed = POLOLU_SPEED_FAST_Y;
  } else if (speed == 1) {
    pocoluSpeed = POLOLU_SPEED_SLOW_Y;
  } else {
    pocoluSpeed = 0;
  }
...
void readUserInputs() {
  // TODO add debouncing
  trackingRequested  = !digitalRead(DI_MODE_AUTO);  //pull-up resistor, switch pulls voltage down at limit
